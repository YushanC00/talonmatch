// @ts-nocheck — test infra: vi.mocked casting not needed, props tested via partial renders
...
describe('UserMenu — guest mode (no user)', () => {
  it('renders dashed guest trigger when no user', () => {
    wrap(<UserMenu parsedResume={RESUME_WITH_SKILLS} onSignOut={vi.fn()} />);
  ...
  it('hides skill count badge when no skills', () => {
    wrap(<UserMenu parsedResume={{ skills: [], experience: [] }} onSignOut={vi.fn()} />);
    fireEvent.click(screen.getByTestId('guest-avatar-trigger'));
  ...
  it('shows Upload résumé button', () => {
    wrap(<UserMenu parsedResume={{ skills: [], experience: [] }} onSignOut={vi.fn()} onNewResume={vi.fn()} />);
    fireEvent.click(screen.getByTestId('guest-avatar-trigger'));
    expect(screen.getByRole('button', { name: /upload résumé/i })).toBeInTheDocument();
  });

  it('Upload résumé calls onNewResume and closes dropdown', () => {
    const onNewResume = vi.fn();
    wrap(<UserMenu parsedResume={{ skills: [], experience: [] }} onSignOut={vi.fn()} onNewResume={onNewResume} />);
    fireEvent.click(screen.getByTestId('guest-avatar-trigger'));
    fireEvent.click(screen.getByRole('button', { name: /upload résumé/i }));
    expect(onNewResume).toHaveBeenCalledOnce();
...
describe('UserMenu — avatar', () => {
  it('renders img when avatar_url present', () => {
    wrap(<UserMenu user={SUPABASE_USER} onSignOut={vi.fn()} />);
  ...
  it('renders initials fallback when avatar_url is empty', () => {
    wrap(<UserMenu user={NO_AVATAR_USER} onSignOut={vi.fn()} />);
...
describe('UserMenu — dropdown', () => {
  ...
  it('"Sign Out" calls supabase.auth.signOut and onSignOut', async () => {
    ...
    wrap(<UserMenu user={SUPABASE_USER} onSignOut={onSignOut} />);
    fireEvent.click(screen.getByRole('button', { name: /user menu/i }));
