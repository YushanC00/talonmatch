...
export default function GoogleSignIn({ parsedResume, onLogin }: GoogleSignInProps) {
  ...
  const handleLogin = async () => {
    ...
    if (oauthError) {
      setError(
        oauthError.message?.includes('not enabled')
          ...
          : (oauthError.message ?? 'Sign-in failed. Please try again.')
      );
      setLoading(false);
      return;
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        if ((parsedResume?.skills?.length ?? 0) > 0 && parsedResume) {
